...
import {getStrDate, isNull} from '~/utils';
    ...
    <TouchableOpacity
      ...
      <View style={{flexDirection: 'row', alignItems: 'center'}}>
        <View style={{flex: 1}}>
          ...
          <Text style={[styles.content, {color: '#7C7C7C', marginTop: 5}]}>{item?.SchoolName}</Text>
        </View>
        <Image
          resizeMode="contain"
      ...
      {show && (
        <>
          ...
          />
          <CardItem title="Ngày bảo lưu: " content={`${getStrDate(item?.CreatedDate)}`} />
          <CardItem title="Hạn bảo lưu: " content={`${getStrDate(item?.DeadLineReserve)}`} />
